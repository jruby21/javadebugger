               ...
               (cons
                (cond
                 ((and (string= lm "main") (not (string= ln "44")))
                  (format "break test.foo 44;break test.foo sum;next %s;next %s;next %s;locals * %s 0;continue"
                          jbug-testThread jbug-testThread jbug-testThread  jbug-testThread))
                 ((and (string= lm "main") (string= ln "44"))
                  (format "locals * %s 0;stack %s;next %s;next %s;into %s;next %s;next %s;next %s;classes;stack %s;back %s;stack %s;continue"
