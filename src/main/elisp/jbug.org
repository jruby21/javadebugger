...
key to a hash table (jbug-commandHashMap), the entry in the hash table describes the
...
The commands are kept in a hash table called jbug-commandHashMap. The table is
...
The jbug-commandHashMap is filled with commands below.
...
The tokens in a response are seperated by spaces.
...
(defun jbug-addResponseTable  (name env)
  (cond ((null name)    (message "Tried to add response table with null name"))
        ((null env)     (message "Tried to add a null response"))
        ((member name (mapcar `jbug-getResponseTableName jbug-responseTables))
  ...
  (jbug-addResponseTable
   ...
   (ht
    ...
    (jbug-accessWatchpointSet-response
     `(lambda (env response)
        (let ((field  (nth 1 response))
              (class (nth 2 response)))
          (writeStringToBuffer
           proc
           (concat
    ...
    (jbug-accessWatchpoint-response
     `(lambda (env response)
        (let ((breakId     (nth 1 response))
              (tr                  (-slice response 2 8))
              (loc               (-slice response 8))
              (className (nth 11 response))
              (fieldName  (nth 12 response))
              (value          (nth 0 (read-from-string (nth 13 response)))))
          (reportBreak (concat "Access watchpoint " breakId " entered") tr loc)
    ...
    (jbug-modificationWatchpoint-response
     `(lambda (env response)
        (let
            ((className              (nth 1 response))
             (fieldName               (nth 2 response))
             (past                         (nth 0 (read-from-string (nth 3 response))))
             (future                      (nth 0 (read-from-string (nth 4 response)))))
          (writeStringToBuffer jbug-proxyProcess "Modification watchpoint\n")
          (writeOrgTableToBuffer
           jbug-proxyProcess
