...
and handles the JDI interface.  The proxy is a sub process to emacs. This elisp
program sends commands to the proxy on stdin and receives replies on stdout.
...
A response function can send one or more new commands to the proxy. Sending
commands immediately, after some of the response functions have executed but
before others have completed, or while a list of commands is queued to be sent to
the proxy, can result in the most remarkable mess. So, response functions add
proxy commands to a special queue (jbug-responseCommands) and when all responses
are done jbug-insertion-filter will queue these commands for execution.
...
The proxy sends strings to this elisp program through stdout. EMACS passes the
string this program by calling the function jbug-insertion-filter.  That
function can be called at any time. It may return a whole string transmitted by
the proxy or only a fragment of the string so we have to buffer the string
returned. A full response string is ended by a new line. We may receive multiple
response strings in a single call to jbug-insertion-filter.
...
STRING is the input from the proxy"
  ...
  (let ((com (split-string jbug-receivedFromProxy "\n" 't)))
    ...
    ; Check if responses are done. If so,  it may be time to execute a queued command.
    ; Sometimes responses will queue commands to be executed. Add them to queue
    ; of commands waiting to execute when everything has calmed down.
