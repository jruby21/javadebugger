...
(ht-set javadebug-response-hooks
   ...
   (lambda (response) (run-hooks 'javadebugger-accessWatchpointSet-hook )))
...
;; accessWatchpointSet
(add-hook 'javadebugger-accessWatchpointSet-hook (lambda ()
...
(defconst commandHashMap
  ((lambda (x)
     (let ((h (ht-create)))
       ...
       h))
  (list
   (javadebug-makeCommandHashEntry "access"
                               (lambda (c) (/= (length c) 3))
