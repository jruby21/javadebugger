...
(defvar defaultFrame  "" "the frame we examined")
...
for an entry in the javadebug-response-hooks hash table.
...
variable. The value in the javadebug-response-hooks table is a function which
...
(ht-set javadebug-response-hooks
   ...
   (lambda (response)
       (run-hook-with-args 'javadebugger-threadList-hook
...
look up the response's entry in the javadebug-response-hooks hash map. If the
...
(defconst
  ...
  "accesswatchpointset" "")
(defvar javadebugger-accessWatchpointSet-hook nil)
(setq javadebugger-accessWatchpointSet-hook ())

(ht-set
 javadebug-response-hooks
 javadebugger-accessWatchpointSet-response
 (lambda (response)
   (run-hooks 'javadebugger-accessWatchpointSet-hook )))

(add-hook
 'javadebugger-accessWatchpointSet-hook (lambda ()
...
(ht-set javadebug-response-hooks
        javadebugger-accessWatchpoint-response
        (lambda (response)
          (run-hook-with-args 'javadebugger-accessWatchpoint-hook
                              (nth 1 response)
                              (nth 2 response)
...
(defvar javadebugger-arguments-hook nil)
(setq javadebugger-arguments-hook ())
(ht-set javadebug-response-hooks
   javadebugger-arguments-response
   (lambda (response) (run-hook-with-args 'javadebugger-arguments-hook (nth 0 (read-from-string (nth 1 response))))))

(add-hook 'javadebugger-arguments-hook
          (lambda (th)
                (writeOrgTableToBuffer
                 proxyProcess
                 "| Name | Value |\n"
...
(ht-set javadebug-response-hooks
   ...
   (lambda (response) (run-hook-with-args 'javadebugger-fields-hook  (nth 1 response) (-partition-in-steps 8 8 (-slice response 2)))))
...
(ht-set javadebug-response-hooks
   ...
   (lambda (response) (run-hook-with-args 'javadebugger-classPrepared-hook response)))

(add-hook 'javadebugger-classPrepared-hook (lambda ( resp)
...
(defvar javadebugger-modificationWatchpointSet-hook nil)
(setq javadebugger-modificationWatchpointSet-hook ())
(ht-set javadebug-response-hooks
   javadebugger-modificationWatchpointSet-response
   (lambda (response) (run-hooks 'javadebugger-modificationWatchpointSet-hook )))

(add-hook 'javadebugger-modificationWatchpointSet-hook (lambda ()
...
(ht-set javadebug-response-hooks
   ...
   (lambda (response)
     (run-hook-with-args 'javadebugger-modificationWatchpoint-hook
                         (nth 1 response)
                         (nth 2 response)
                         (nth 0 (read-from-string (nth 3 response)))
...
(defvar javadebugger-preparingClass-hook nil)
(setq javadebugger-preparingClass-hook ())
(ht-set javadebug-response-hooks
   javadebugger-preparingClass-response
   (lambda (response) (run-hook-with-args 'javadebugger-preparingClass-hook  (nth 1 response))))

(add-hook 'javadebugger-preparingClass-hook (lambda (name)
...
;; log
(add-hook 'javadebugger-log-hook (lambda ( resp)
            (writeStringToBuffer proc (concat  (s-join " " resp) ".\n"))))
